...
/**
 * ПОЛУШИРИНА собранной фигурки, units — замер по её геометрии, а не вывод из
 * капсулы.
 *
 * Нужна там, где габарит должен совпадать с тем, что видит игрок: хитбокс
 * попаданий и точка опоры падающего тела. Из капсулы это не выводится — фигурка
 * вдвое уже: у босса 0.83 против радиуса капсулы 1.6, у обычного зомби 0.21
 * против 0.3.
 *
 * Берётся max|x|, а не полуразмах: фигурка по x симметрична, но если когда-нибудь
 * перестанет быть, габарит должен покрывать её целиком. Глубина (по z) в расчёт
 * не идёт — по ней фигурка длиннее из-за протянутых рук, а круглому хитбоксу и
 * лежащему телу нужна толщина корпуса.
 */
export function figureHalfWidth(geometry: BufferGeometry): number {
  geometry.computeBoundingBox();
  const box = geometry.boundingBox!;
  return Math.max(Math.abs(box.min.x), box.max.x);
}
