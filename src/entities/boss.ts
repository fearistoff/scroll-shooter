...
export class Boss {
   ...
   * (CONFIG.run.waveHpGrowth), округлённый вверх до кратного layerHp. Число слоёв
   * полосы считается от него делением на постоянную толщину (layerHp), поэтому у
   * босса поздней волны слоёв больше, и все они полные.
   */
  private maxHp = 0;
  /**
  ...
  /**
   * Сколько слоёв было у ЭТОГО босса на выходе. Запас в spawn() уже округлён до
   * кратного layerHp, поэтому слои полные и деление точное. Ceil оставлен на
   * случай, когда layerHp покрутили и кратность сломалась: неполный верхний слой
   * всё равно слой — иначе его hp снимались бы «вне полосы» и за них никто не
   * заплатил бы.
   */
  ...
  spawn(): void {
    ...
    this.phase = 'entering';
    // Босс крепнет с волной наравне с зомби (CONFIG.run.waveHpGrowth), но запас
    // округляется ВВЕРХ до целого числа слоёв полосы (CONFIG.boss.layerHp = 100).
    // Множитель волны дробный (1.2^(волна−1)), и без округления верхний слой
    // выходил неполным: у босса десятой волны 5159.78 hp — 51 полный слой и
    // 59.78 сверху, из-за чего «×52» пропадало с первых же 60 урона. С
    // округлением каждый слой стоит одинаково и по времени, и по деньгам
    // (money.perBossLayer), а полное число слоёв всегда честное.
    const step = CONFIG.boss.layerHp;
    const scaled = totalHp * this.run.hpMultiplier;
    this.maxHp = step > 0 ? Math.ceil(scaled / step) * step : scaled;
    this.hp = this.maxHp;
