 ...
 * Одета по требованию: куртка защитного цвета, тёмно-серые штаны, чёрные
 * ботинки и волосы; автомат нарисован в руках. Порядок фигур смысловой:
 * кисти ложатся ПОВЕРХ автомата (иначе он лежал бы на руках), лицо поверх
 * волос — так затылок с макушкой остаются тёмной полосой.
 *
 * Голова УГЛОВАТАЯ, по эскизу пользователя (2026-08-04): лицо — трапеция,
 * СУЖАЮЩАЯСЯ к подбородку, а волосы — скошенный клин ПОВЕРХ него: накрывают
 * макушку, спускаются по затылку до шеи, и их нижний срез идёт диагональю от
 * затылка к линии лба — открытой остаётся передне-нижняя часть лица. Поэтому
 * здесь волосы рисуются ПОСЛЕ лица, в отличие от кистей и автомата ниже.
 * Та же форма у 3D-модели бойца (entities/soldier.ts).
 *
...
const SHOOTER_FIGURE =
  '<path class="fig-skin" d="M6.7 2h2.5l-.6 2.85h-1.1z"/>' +
  '<path class="fig-black" d="M9.2 2.1 7.4 1.15 5.5 1.75l-.4 2.8z"/>' +
