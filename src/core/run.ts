...
export class RunState {
   ...
   * разгон плотности и секундные разблокировки (run.unlocks) стартуют как в
   * обычной вылазке. Куплен только пропуск ранних волн, а не перемотка забега;
   * ступени оружия при этом ждать не приходится — их замки считаются
   * убийствами текущей волны (unlocks.weapons.kills), а не секундами.
   */
   ...
   * упёрлись в потолки (при текущих числах — с волны 9), состав случайный
   ...
   * 1, 1.2, 1.44, … (CONFIG.run.waveExpGrowth — тот же темп, которым растут
   * прочность и урон противников).
   *
