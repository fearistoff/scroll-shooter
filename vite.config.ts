...
export default defineConfig({
  ...
  server: {
    ...
    open: false,
    /*
     * Горячая замена ОТКЛЮЧЕНА намеренно (решение пользователя): игра держит
     * состояние в замкнутом цикле (__game, пулы, подписки на window), и HMR
     * подменял модули под живым состоянием — после правки на экране могла
     * оказаться смесь старого и нового кода. Правило простое: проверяешь
     * правку — перезагрузи страницу; свежие модули dev-сервер отдаёт как
     * обычно, автоматической перезагрузки просто нет.
     */
    hmr: false,
  },
  build: {
